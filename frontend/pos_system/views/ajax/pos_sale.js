...
<?php
// Generar códigos de control DTE (dinámicos por emisión)
...
function generarNumeroControl() {
    ...
    return 'DTE-01-' . strtoupper(substr(bin2hex(random_bytes(8)), 0, 15 - strlen((string)$last))) . $last;
}
...
const mhControl = {
    codigoGeneracion: "<?php echo $codigoGeneracion; ?>",
    numeroControl: "<?php echo $numeroControl; ?>",
    fechaEmision: "<?php echo date('Y-m-d'); ?>",
    horaEmision: "<?php echo date('H:i:s'); ?>"
};

// Estado global del sistema
let localCart = [];
let cartDiscount = { type: null, amount: 0 }; // Initialize cart discount // Initialize cart discount
let currentCustomer = {
    nombre: "Consumidor Final", // Customer name // Customer name
    correo: "damefactura@gmail.com",
    tipoDocumento: "13",
    numDocumento: "00000000-0",
    direccion: { 
        departamento: "06", // Department code // Department code 
        municipio: "23", 
...
async function handleBarcodeInput(barcode) {
    if (!barcode || barcode.trim() === "") return; // Check for empty barcode
    
    try {
        const response = await fetch('ajax/search_product.php', { // Fetch product data
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ barcode: barcode.trim() }) // Send barcode to server
        });
