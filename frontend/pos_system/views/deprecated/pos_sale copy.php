...
* SISTEMA POS - AUDITADO v3.5 
 * Ajustes: Atajos de teclado, Búsqueda automática y Fix de Buscador
 */
...
// --- 1. LÓGICA DE BÚSQUEDA Y CARRITO ---

/**
 * Función principal para procesar el código de barras
 */
async function handleBarcodeInput(barcode) {
    ...
    try {
        ...
        if (result.success && result.product) {
            ...
            // Limpiar input y mantener foco para la siguiente lectura
            if(input) input.value = '';
        } else {
            // Sonido de error o notificación breve
            Swal.fire({
                title: 'No encontrado',
                text: `El código ${barcode} no existe.`,
                icon: 'warning',
                timer: 1500,
...
function renderLocalCart() {
    ...
    if (cartDiscount.type === 'percentage') {
        discountValue = subtotal * (cartDiscount.amount / 100);
    } else {
        discountValue = cartDiscount.amount;
    }

    const finalTotal = Math.max(0, subtotal - discountValue);

    if(subtotalDisplay) subtotalDisplay.textContent = `$${subtotal.toFixed(2)}`;
    totalDisplay.innerHTML = discountValue > 0 ? 
        `<small class="text-danger fs-6">-${discountValue.toFixed(2)}</small> $${finalTotal.toFixed(2)}` : 
...
document.addEventListener('keydown', function(e) {
    // Evitar que las teclas F1-F12 realicen funciones del navegador
    if (['F1', 'F7', 'F8', 'F9', 'F10', 'Escape'].includes(e.key)) {
        e.preventDefault();
    }

    switch(e.key) {
        case 'F1':      openCustomerModal(); break;
        case 'F7':      if(!document.getElementById('finish-sale-btn').disabled) processSale(); break;
        case 'F8':      applyDiscount(); break;
        case 'F9':      openPaymentModal('cash'); break;
        case 'F10':     openPaymentModal('card'); break;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (input) {
        input.focus();
        // El lector de barras suele enviar un 'Enter' al final
        ...
        // Mantener el foco en el input siempre (opcional para agilizar)
        document.addEventListener('click', () => {
            // Solo re-enfocar si no hay un modal abierto
            if (!document.querySelector('.modal.show') && !document.querySelector('.swal2-shown')) {
                input.focus();
            }
        });
    }
