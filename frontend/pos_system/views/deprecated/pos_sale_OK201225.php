...
async function processSale() {
    ...
    Swal.fire({
        ...
        text: 'Enviando a firma electrónica',
        allowOutsideClick: false,
        didOpen: () => { Swal.showLoading(); }
    });

    try {
        const response = await fetch('ajax/save_dte.php', {
            method: 'POST',
        ...
        if (result.success) {
            await Swal.fire('Éxito', 'Venta Finalizada Correctamente', 'success');
            
            // Abrir Ticket
            const idGen = result.codigoGeneracion || mhControl.codigoGeneracion;
            window.open(`print_ticket.php?id=${idGen}`, '_blank', 'width=300,height=600');
            
            location.reload(); // Reiniciar para nueva venta
        } else {
            Swal.fire('Error', result.error || 'Error al guardar DTE', 'error');
        }
    } catch (error) {
