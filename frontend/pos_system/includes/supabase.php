 ...
 *
 * Mantiene la API histórica `supabase($table)` para no tener que reescribir
 * todos los controladores del POS de una sola vez. NO realiza llamadas a
 * Supabase ni PostgreSQL.
 */
...
final class SupabaseClient
{
    ...
    private const TABLE_MAP = [
        'empresas' => 'companies',
        'usuarios' => 'users',
        'pos_products' => 'dte_productos',
        'pos_productos' => 'dte_productos',
        'pos_clientes' => 'mh_cliente_consumidor',
        'clientes' => 'mh_cliente_consumidor',
        'pos_proveedores' => 'mh_proveedor_contribuyente',
        'proveedores' => 'mh_proveedor_contribuyente',
        'pos_ventas' => 'pos_ventas',
        'pos_sales' => 'pos_sales',
        'pos_venta_detalles' => 'detalle_ventas',
        'pos_sale_items' => 'detalle_ventas',
    ];
    ...
    {
        $this->requireTable();
        return self::TABLE_MAP[$this->table] ?? $this->table;
    }
    ...
    public function select(string $columns = '*', array $filters = [], ?int $limit = null, ?int $offset = null): array
    {
        ...
        $sql = 'SELECT ' . $this->sanitizeColumns($columns) . ' FROM ' . $this->qi($table);
        $params = [];
        ...
        try {
            ...
            $stmt->execute($params);
            return ['success' => true, 'data' => $stmt->fetchAll(), 'count' => $stmt->rowCount()];
        } catch (Throwable $e) {
    ...
    public function insert(array $data): array
    {
        ...
        // Si la tabla tiene INTEGER PRIMARY KEY AUTOINCREMENT, SQLite debe generar id.
        // Solo generamos UUID cuando la tabla usa un id TEXT o cuando el caller lo entrega.
        if (!array_key_exists('id', $data) && !$this->integerPrimaryKey($table)) {
            $data['id'] = $this->uuid();
        }

        $this->ensureColumns($table, array_keys($data));
        $columns = array_keys($data);
        $params = [];
        foreach ($columns as $column) {
            $params[':v_' . $column] = $this->normalize($data[$column]);
        }
        ...
        try {
            ...
            if ($this->hasColumn($table, 'id')) {
                $lookup = array_key_exists('id', $data) ? $data['id'] : $id;
                if ($lookup !== '' && $lookup !== '0' && $lookup !== null) {
                    $q = $this->pdo->prepare('SELECT * FROM ' . $this->qi($table) . ' WHERE id = ? LIMIT 1');
                    $q->execute([$lookup]);
                    $row = $q->fetch() ?: [];
                }
            }
    ...
    public function update(array $data, array $filters = []): array
    {
        $table = $this->physicalTable();
        $this->ensureTable($table);
        if (!$data) return ['success' => true, 'data' => [], 'affected' => 0];
        $this->ensureColumns($table, array_keys($data));
        ...
        try {
            $stmt = $this->pdo->prepare('UPDATE ' . $this->qi($table) . ' SET ' . implode(', ', $sets) . ' WHERE ' . $where);
            $stmt->execute($params);
    ...
    public function delete(array $filters = []): array
    {
        ...
        try {
            $stmt = $this->pdo->prepare('DELETE FROM ' . $this->qi($table) . ' WHERE ' . $where);
            $stmt->execute($params);
            return ['success' => true, 'data' => [], 'affected' => $stmt->rowCount()];
        } catch (Throwable $e) {
            return ['success' => false, 'data' => [], 'error' => $e->getMessage()];
    ...
    {
        // Bloquear restos de SQL específico de PostgreSQL en la ruta POS.
        if (preg_match('/\b(jsonb|bigserial|uuid_generate|FOR\s+UPDATE|ILIKE|RETURNING\s+\*)\b/i', $sql)) {
            return ['success' => false, 'data' => [], 'error' => 'Consulta PostgreSQL no compatible con SQLite'];
        }
        try {
    ...
    public function insertDTE(array $dteData, ?string $usuarioId = null, ?string $empresaNit = null): array
    {
        ...
        try {
            return $this->insert([
                ...
                'documento_json' => json_encode($dteData, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
    ...
    {
        return ['success' => false, 'data' => [], 'error' => 'RPC externo eliminado: ' . $functionName];
    }

    private function ensureTable(string $table): void
    {
        if (!$this->tableExists($table)) {
            $this->pdo->exec('CREATE TABLE ' . $this->qi($table) . ' (id INTEGER PRIMARY KEY AUTOINCREMENT)');
        }
    }
    ...
    {
        foreach ($columns as $column) {
            if (!preg_match('/^[A-Za-z_][A-Za-z0-9_]*$/', (string)$column)) throw new InvalidArgumentException('Invalid column: ' . $column);
            if (!$this->hasColumn($table, (string)$column)) {
                $this->pdo->exec('ALTER TABLE ' . $this->qi($table) . ' ADD COLUMN ' . $this->qi((string)$column) . ' TEXT');
            }
        }
    }
    ...
    private function integerPrimaryKey(string $table): bool
    {
        $stmt = $this->pdo->query('PRAGMA table_info(' . $this->qi($table) . ')');
        foreach ($stmt->fetchAll() as $row) {
            if ($row['name'] === 'id') return strtoupper((string)$row['type']) === 'INTEGER' && (int)$row['pk'] === 1;
        }
        return false;
    }
        ...
        foreach ($filters as $column => $value) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $v) {
                    $p = ':filter_' . $i++;
                    $placeholders[] = $p;
                    $params[$p] = $this->normalize($v);
                }
                if ($placeholders) $where[] = $this->qi((string)$column) . ' IN (' . implode(',', $placeholders) . ')';
            } else {
                $p = ':filter_' . $i++;
                $where[] = $this->qi((string)$column) . ' = ' . $p;
                $params[$p] = $this->normalize($value);
            }
        }
    ...
    private function sanitizeColumns(string $columns): string
    {
        ...
        foreach (array_map('trim', explode(',', $columns)) as $part) {
            if (!preg_match('/^[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)?(\s+AS\s+[A-Za-z_][A-Za-z0-9_]*)?$/i', $part)) {
                throw new InvalidArgumentException('Invalid column expression');
            }
        }
        return $columns;
    }
    ...
    {
        if (!$this->table) throw new RuntimeException('Table name is not set');
    }
...
{
    static $instances = [];
    $key = $table ?? '__default__';
    if (!isset($instances[$key])) $instances[$key] = new SupabaseClient($table);
