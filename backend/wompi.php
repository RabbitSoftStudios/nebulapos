...
if(empty($_SESSION['pos_authenticated']) || empty($_SESSION['empresa_id'])){http_response_code(401);echo json_encode(['status'=>'error','message'=>'Sesión POS no autenticada.']);exit;}
processPayment($pdo,$baseUrl,$publicKey,$privateKey,$data,(int)$_SESSION['empresa_id']);
...
function processPayment(PDO $pdo,string $baseUrl,string $publicKey,string $privateKey,array $data,int $companyId):void{
    $amount=(int)($data['amount_in_cents']??0);$currency=strtoupper(trim((string)($data['currency']??'USD')));$email=trim((string)($data['customer_email']??''));$saleId=isset($data['sale_id'])?(int)$data['sale_id']:null;
    ...
    if($saleId!==null){
        $sale=$pdo->prepare('SELECT id,company_id,total FROM pos_sales WHERE id=? AND company_id=? LIMIT 1');
        $sale->execute([$saleId,$companyId]);
        if(!$sale->fetch()){http_response_code(403);echo json_encode(['status'=>'error','message'=>'La venta no pertenece a la empresa de la sesión.']);return;}
    }
    ...
    $stmt=$pdo->prepare('INSERT INTO transactions (company_id,transaction_id,reference,amount_in_cents,currency,customer_email,status,payment_method_type,sale_id,wompi_response) VALUES (?,?,?,?,?,?,?,?,?,?)');
    $stmt->execute([$companyId,$txId!==''?$txId:null,$reference,$amount,$currency,$email,$status,'CARD',$saleId,json_encode($tx['data']??$tx,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)]);
    ...
    if($saleId!==null){$pay=$pdo->prepare('INSERT INTO payments (company_id,sale_id,method,amount,status,reference) VALUES (?,?,?,?,?,?)');$pay->execute([$companyId,$saleId,'card',$amount/100,'approved',$reference]);}
